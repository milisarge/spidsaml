...
func (response *Response) Validate(inResponseTo string) error {
	...
	// cancelled it i got destination does not match error
	...
	//knownDestination := destination == response.SP.AcsUrl
	...
	if response.Success() {
		// We expect to have an <Assertion> element or EncryptedAssertion
		encAssertion := response.doc.FindElement("/Response/EncryptedAssertion")

		if encAssertion != nil {
		    // Get the EncryptedData element
		    encData := encAssertion.FindElement("EncryptedData")
		    if encData == nil {
		        return fmt.Errorf("EncryptedAssertion found but no EncryptedData")
		    }
		    // Decrypt
		    decryptedXML, err := response.SP.decryptElement(encData)
		    if err != nil {
		        return fmt.Errorf("failed to decrypt EncryptedAssertion: %s", err.Error())
		    }

		    assertionDoc := etree.NewDocument()
		    if err := assertionDoc.ReadFromString(decryptedXML); err != nil {
			return fmt.Errorf("XML stringi ayrıştırılamadı: %w", err)
		    }
		    assertionRoot := assertionDoc.Root()

        	    if assertionRoot == nil {
			return fmt.Errorf("ayrıştırılan XML'de kök öğe bulunamadı")
		    }
		    response.doc.Root().AddChild(assertionRoot.Copy())

		    removedToken := response.doc.Root().RemoveChildAt(encAssertion.Index())
		    if removedToken == nil {
		        // Bu, normalde olmamalıdır çünkü FindElement başarılı oldu.
		        return fmt.Errorf("EncryptedAssertion öğesi alt öğe listesinden kaldırılamadı")
		    }

		    // Refresh the doc field (because the DOM changed)
  		    //respXML, _ := response.doc.WriteToString()
		}
...
func (response *Response) Attributes() map[string]string {
	...
	for _, e := range response.doc.FindElements("/Response/Assertion/AttributeStatement/Attribute") {
		attributes[e.SelectAttr("FriendlyName").Value] = e.FindElement("AttributeValue").Text()
	}
	return attributes
}
