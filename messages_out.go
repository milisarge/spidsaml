...
func _SignXML(xml []byte, sp *SP) ([]byte, error) {
    ...
    // Find placeholder
    placeholder := doc.FindElement("//ds:Signature")
    if placeholder == nil {
    ...
    // Sign returns a *NEW* element
    signedRoot, err := signingContext.SignEnveloped(doc.Root())
    if err != nil {
        return nil, err
    }

    // Extract actual Signature element safely
    sigEl := signedRoot.FindElement(".//ds:Signature")
    if sigEl == nil {
        return nil, errors.New("real signature not found")
    }

    // IMPORTANT:
    // 1) sigEl MUST be cloned before inserting into another tree
    // 2) etree Clone() fixes the "bad attribute" corruption issue
    cloned := sigEl.Copy()

    // Insert back where placeholder was
    parent.InsertChildAt(index, cloned)
